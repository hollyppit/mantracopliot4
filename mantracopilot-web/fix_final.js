...
const tPath = 'e:/만트라 스튜디오 운영/앱 개발 소스/만트라 코파일럿_프로토타입/temp_full.txt';
const iPath = 'e:/만트라 스튜디오 운영/앱 개발 소스/만트라 코파일럿_프로토타입/index.html';

try {
  const lines = fs.readFileSync(tPath, 'utf8');

  const fixedSteps = `  { id:'s-price', visual:'💰', stageIntro:null, question:'스토어 판매 가격을 설정해주세요.', example:'예시: 1500원 정도로 가볍게 책정, 4900원(프리미엄)', choices:['무료 배포','990원','2,900원','4,900원'], fieldId:'s-price' }
];
...
`;
  ...
  const s_point = "fieldId:'s-desc' },";
  const f_start = "async function fetchAIFeedback(userInput, stepInfo) {";

  const idx = lines.indexOf(s_point);
  const idx2 = lines.indexOf(f_start);

  if (idx !== -1 && idx2 !== -1) {
    let content = lines.substring(0, idx + s_point.length) + '\n' + fixedSteps + coreFuncs + lines.substring(idx2);
    
    if (!content.includes('</html>')) {
      content += '\n</script>\n</body>\n</html>';
    }

    fs.writeFileSync(iPath, content, 'utf8');
    console.log('Success');
  } else {
    console.error('Failed to find split points', { idx, idx2 });
  }
} catch (e) {
  console.error(e);
}
