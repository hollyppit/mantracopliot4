...
path = r"e:\만트라 스튜디오 운영\앱 개발 소스\만트라 코파일럿_프로토타입\index.html"

try:
    with open(path, "r", encoding="utf-8") as f:
        lines = f.readlines()
except Exception as e:
    # Try with cp949 (Korean Windows default) if utf-8 fails
    try:
...
for i in range(min(1802, len(lines))):
    new_lines.append(lines[i])

# ACT_MAP definition
new_lines.append("const ACT_MAP = {\n")
new_lines.append("  desire: ['l-1','l-2','l-3','l-3b','l-3c','l-4','l-5','l-6'],\n")
new_lines.append("  conflict: ['l-7','l-8','l-9','l-10','l-11','l-12','l-13','l-14'],\n")
new_lines.append("  overcome: ['l-15','l-16','l-17','l-18','l-19','l-20']\n")
new_lines.append("};\n")
new_lines.append("let isSeller = false;\n")

# Part 2: Lines 1804 up to 1889
for i in range(1803, min(1889, len(lines))):
    new_lines.append(lines[i])

# New SellerSteps and Logic (Regular string, no f-string)
repl = """const sellerSteps = [
...
async function fetchAIFeedback(userInput, stepInfo) {
  ...
  } else if (selectedCharName === '설정학 전문가') { persona = "당신은 가상세계 설정 전문가입니다."; }
  else if (selectedCharName === '냉정한 편집자') { persona = "당신은 10년 경력의 웹툰/웹소설 편집자입니다."; }
  else if (selectedCharName === '작품 편집자') { persona = "당신은 작가님의 전담 편집자입니다."; }
...
async function processUserAnswer(val) {
  ...
  const txtField = document.getElementById('dialogTxt');
  if(txtField) txtField.innerHTML = '<span style="color:var(--text-dim);">(처리 중...)</span>';
...
function showReplyFlash(text) {
  const el = document.getElementById('replyFlash');
  if (!el) return;
...
# Ensure tags
if not any("</html>" in line for line in new_lines[-5:]):
    if not any("</script>" in line for line in new_lines[-5:]):
        new_lines.append("</script>\n")
    if not any("</body>" in line for line in new_lines[-5:]):
